...
def mlat_iter(rel_stations, prange_obs, xguess=[0, 0, 0], limit=20, maxrounds=100):
    ...
    while numpy.linalg.norm(xerr) > limit:
        ...
        H = numpy.array(
            [(numpy.array(-rel_stations[row, :]) + xguess) / prange_est[row] for row in range(0, len(rel_stations))])
...
def mlat(replies, altitude):
    ...
    prange_obs.append(
        [numpy.linalg.norm(llh2ecef((me_llh[0], me_llh[1], altitude)))])  # use ECEF not geoid since alt is MSL not GPS
...
if __name__ == '__main__':
    ...
    teststations = [[18.530143, 73.854764, 0], [18.530835, 73.856097, 0], [18.530637, 73.857547, 0], [18.527260, 73.854601, 0]]
    testalt = 8000
