...
import requests
from colours import *
from global_variables import apikey
...
class GoogleMapPlotter(object):
    ...
    def draw_to_string(self):
        ...
        strg += ('<title>Signal Recorder Estimations</title>\n')
        strg += (
            '<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key='+ apikey +'&callback=initMap&libraries=visualization&sensor=true_or_false"></script>\n')
        strg += ('<script type="text/javascript">\n')
        strg += ('\tfunction initialize() {\n')
        try:
            strg += self.write_map_str()
        except TypeError:
            pass
        try:
            strg += self.write_grids_str()
        except TypeError:
            pass
        try:
            strg += self.write_points_str()
        except TypeError:
            pass
        try:
            strg += self.write_paths_str()
        except TypeError:
            pass
        try:
            strg +=  self.write_shapes_str()
        except TypeError:
            pass
        try:
            strg += self.write_heatmap_str()
        except TypeError:
            pass
    ...
    def write_grids_str(self):
        ...
        for line in self.grids:
            ...
            strg = self.write_polyline_str(line, settings)
        return strg
    ...
    def write_map_str(self):
        strg = ""
        strg += ('\t\tvar centerlatlng = new google.maps.LatLng(%f, %f);\n' %
                 (self.center[0], self.center[1]))
        strg +=('\t\tvar myOptions = {\n')
        strg +=('\t\t\tzoom: %d,\n' % (self.zoom))
        strg +=('\t\t\tcenter: centerlatlng,\n')
        strg +=('\t\t\tmapTypeId: google.maps.MapTypeId.ROADMAP\n')
        strg +=('\t\t};\n')
        strg +=(
    ...
    def write_point_str(self, lat, lon, color, title):
        strg = ""
        strg +=('\t\tvar latlng = new google.maps.LatLng(%f, %f);\n' %
                (lat, lon))
        strg +=('\t\tvar img = new google.maps.MarkerImage(\'%s\');\n' %
                (self.coloricon % color))
        strg +=('\t\tvar marker = new google.maps.Marker({\n')
        strg +=('\t\ttitle: "%s",\n' % title)
        strg +=('\t\ticon: img,\n')
        strg +=('\t\tposition: latlng\n')
        strg +=('\t\t});\n')
        strg +=('\t\tmarker.setMap(map);\n')
        strg +=('\n')
        return strg
    ...
    def write_polyline_str(self, path, settings):
        ...
        strg +=('var Path = new google.maps.Polyline({\n')
        strg +=('clickable: %s,\n' % (str(clickable).lower()))
        strg +=('geodesic: %s,\n' % (str(geodesic).lower()))
        strg +=('path: PolylineCoordinates,\n')
        strg +=('strokeColor: "%s",\n' % (strokeColor))
        strg +=('strokeOpacity: %f,\n' % (strokeOpacity))
        strg +=('strokeWeight: %d\n' % (strokeWeight))
        strg +=('});\n')
        strg +=('\n')
        strg +=('Path.setMap(map);\n')
        strg +=('\n\n')
        return strg
    ...
    def write_polygon_str(self, path, settings):
        ...
        strg +=('var polygon = new google.maps.Polygon({\n')
        strg +=('clickable: %s,\n' % (str(clickable).lower()))
        strg +=('geodesic: %s,\n' % (str(geodesic).lower()))
        strg +=('fillColor: "%s",\n' % (fillColor))
        strg +=('fillOpacity: %f,\n' % (fillOpacity))
        ...
        strg +=('});\n')
        strg +=('\n')
        strg +=('polygon.setMap(map);\n')
        strg +=('\n\n')
        return strg

    def write_heatmap_str(self):
        strg = ""
        for heatmap_points, settings_string in self.heatmap_points:
            strg +=('var heatmap_points = [\n')
            for heatmap_lat, heatmap_lng in heatmap_points:
                strg +=('new google.maps.LatLng(%f, %f),\n' %
                        (heatmap_lat, heatmap_lng))
            strg +=('];\n')
            strg +=('\n')
            strg +=('var pointArray = new google.maps.MVCArray(heatmap_points);' + '\n')
            strg +=('var heatmap;' + '\n')
            strg +=('heatmap = new google.maps.visualization.HeatmapLayer({' + '\n')
            strg +=('\n')
            strg +=('data: pointArray' + '\n')
            strg +=('});' + '\n')
            strg +=('heatmap.setMap(map);' + '\n')
            strg +=(settings_string)
            return strg
